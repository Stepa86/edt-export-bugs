...
Процедура ИнициализацияПараметров(Знач Команда)
    ...
    _РезультатПроверки = файлСРезультатомПроверки.ПолноеИмя;
    ...
    _КаталогИсходников = каталогИсходников.ПолноеИмя;
    ...
    _ФайлДжсон = файлДжсон.ПолноеИмя;    
...
Функция ТаблицаПоФайлуРезультата()
	...
	Пока Не ПрочитаннаяСтрока = Неопределено Цикл
        ...
        Если Не ЗначениеЗаполнено( компонентыСтроки[4] ) Тогда
...
Процедура ЗаполнитьВТаблицеРезультатовИсходныеПути( таблицаРезультатов )
    ...
    Для каждого цСтрока Из таблицаРезультатов Цикл
        ...
        цСтрока.Путь = ПутьКИсходнику1С(цСтрока.Метаданные);

        Если Не ЗначениеЗаполнено(цСтрока.Путь) Тогда

            _лог.Ошибка( СтрШаблон( "Путь для <%1> не получен", цСтрока.Метаданные) );

        ИначеЕсли Не ФайлСуществует( цСтрока.Путь ) Тогда

            _лог.Ошибка( СтрШаблон( "Путь <%1> для <%2> не существует", цСтрока.Путь, цСтрока.Метаданные) );
...
Функция ПутьКИсходнику1С( Знач пМетаданные )
